const state = {
    ...
    actions: {
        ...
        randomAmbuT: setInterval(randomSquare, 1000),
    }
}
...
function randomSquare() {
    state.view.squares.forEach((square) => {
        square.classList.remove("enemy","hourglass");
    });
...
function randomAmbu() {
    state.view.squares.forEach((square) => {
        square.classList.remove("ampulheta");
    });
...
function addListenerAmpulheta() {
    state.view.squares.forEach((square) => {
        square.addEventListener("mouseclick", () => {
            if (square.id === state.values.scoreRalph) {
                state.values.result++;
                state.view.scoreRalph.textContent = state.values.result;
                state.values.resultRalph = randomAmbu.id;
                playSound("hit");
            }
        })
    });
